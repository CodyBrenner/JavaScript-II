...
runners.forEach(function(runners){
     fullName.push(`The runners name is ${runners.first_name} ${runners.last_name}`);
...
let emailList = [];
runners.forEach(function(runners){
    emailList.push(`${runners.first_name} Email is:  ${runners.email}`)
});
console.log(emailList);

// Problem 2 Get Company Name from Skinix
let companyName = runners.filter(function(runners){
    return runners.company_name === "Skinix"
});
console.log(companyName);
// Problem 3
let ticketPriceAvg = runners.reduce(function(sum, runners){
    return sum + runners.donation / 50;
}, 0);
